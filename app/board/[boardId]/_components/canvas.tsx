...
import { CursorsPresence } from './cursor-presences'
import {
  colorToCss,
...
export const Canvas = ({ boardId }: CanvasProps) => {
  ...
  const insertPath = useMutation(
    ({ storage, self, setMyPresence }) => {
      ...
      const { pencilDraft } = self.presence
      if (pencilDraft == null || pencilDraft.length < 2 || liveLayer.size >= MAX_LAYERS) {
        setMyPresence({ pencilDraft: null })
        return
      }
      const id = nanoid()
      liveLayer.set(id, new LiveObject(penPointsToPathLayer(pencilDraft, lastUsedColor)))
      const liveLayerIds = storage.get('layerIds')
